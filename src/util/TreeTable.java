	...
	public void setContent(ArrayList<ExperimentData> experimentData) {
		...
		if (experimentData != null && experimentData.size() > 0) {
			...
			for (int i = 0; i < nrOfColumns - 2; i++) {
				headings[2 + i] = "<html><b>"
						+ experiments.get(0).annotations.get(i).name + "</html></b>";
			}
		...
		for (int i = 0; i < rows.length; i++) {
			...
			if(nodeObject instanceof ExperimentNode) {
				...
				ExperimentData newExp = new ExperimentData(
						...
						(ArrayList<AnnotationDataValue>) exp.annotations.clone());
				...
				if(parentNode instanceof ExperimentNode) {
					...
					if(selectedExperiments.contains(tempExp)) {
						...
						if(!selectedExperiments.get(index).files.contains(fileNode.getFile())) {
				...
				/*If support node (file headings node or raw/region/profile node*/
				...
				if(parentNode instanceof ExperimentNode) {
					...
					if(selectedExperiments.contains(tempExp)) {
						...
						for(FileData file : newFiles) {
							if(!selectedExperiments.get(index).files.contains(file)) {
								ArrayList<FileData> newFile = new ArrayList<FileData>();
								newFile.add(file);
								selectedExperiments.get(index).addFiles(newFile);
							}
						...
						ExperimentData exp = new ExperimentData(
								...
								tempExp.createdBy,
								newFiles,
								(ArrayList<AnnotationDataValue>)tempExp.annotations.clone());
						selectedExperiments.add(exp);
					}
				}
			}
