...
public class SysadminController extends Observable {
    ...
    public void editAnnotation() {
        ...
        if (!(oldAnnotation.isForced() == newAnnotation.isForced())) {
            ...
            // model.changeAnnotationForced(newAnnotation.name);
        } else {
            System.out.println("Forced value not changed");
        }
        
        if (newAnnotation.getValues().length == oldAnnotation.getValues().length) { // if
                                                                                    // the
                                                                                    // number
                                                                                    // of
                                                                                    // annotation
                                                                                    ...
                                                                                    // the
                                                                                    // same
            for (int i = 0; i < newAnnotation.getValues().length; i++) { // go
                                                                         // through
                                                                         // list
                                                                         // of
                                                                         // annotations
                                                                         ...
                                                                         // changed
                                                                         // values
                if (!(newAnnotation.getValues()[i].equals(oldAnnotation
                        .getValues()[i]))) {
        ...
        if (newAnnotation.getValues().length > oldAnnotation.getValues().length) { // if
                                                                                   ...
                                                                                   // values
                                                                                   // added
            int noOfAddedValues = newAnnotation.getValues().length
                    - oldAnnotation.getValues().length;
            for (int i = 0; i < newAnnotation.getValues().length; i++) { // go
                                                                         // through
                                                                         // list
                                                                         // of
            ...
            for (int i = 0; i < noOfAddedValues; i++) {
                // model.addAnnotationValue();
            }
        }
